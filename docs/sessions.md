...
1. Call `fl_project_start` with a fresh workspace path, such as `sessions/first.flp`. Its default interactive mode refuses to reuse any already-running FL process. Set `background=true` to launch on a private Windows desktop; independent MCP clients can each own one background session concurrently. Cold background starts for the same executable serialize only through FL's native initialization, and the tool deadline includes time waiting for that startup gate. Ready sessions and renders can run concurrently. Startup waits for a responding bridge and the exact copied project path, then keeps polling until tempo, PPQ and title stop changing (FL reports the template's tempo until the copied project is applied) before returning readiness, tempo, and PPQ; `settle` in the result says how long that took and whether it stabilised.
...
5. Call `fl_project_render` with a fresh output path such as `audio/first.wav`. This saves another snapshot, checks its FLP envelope and complete data chunk, ends only the disposable managed editor, then runs FL's documented command-line WAV export. It requires successful process exit and a structurally nonempty WAV whose length matches the project's span before returning the audio and snapshot paths; a renderer that dies part-way has its partial file moved to `<name>.failed-attempt1.wav` and the same snapshot is rendered once more, and a second failure reports both attempts with the plugin-host log tail. A header check is not proof that every FL event or external asset is valid.
...
Output overwrite, path traversal, Windows device names, alternate data streams, and workspace symlink/junction traversal are refused. Samples are either workspace files (relative paths resolve against the workspace root, `%LOCALAPPDATA%\FlMcp\Projects` by default) or library entries as `list_samples` prints them (`[P]Drums\Kicks\909 Kick.wav` for FL's factory packs, `[U]...` for the user's Image-Line documents content, `[B1]...`/`[B2]...` for the folders FL's browser searches in addition to those (its "extra search folders", plus anything `FRUITYLINK_SAMPLE_ROOTS` adds), or a full path under any of those roots); a library entry is copied into `<workspace>\staged-samples\packs|user|b1|...\...` and the copy is what the channel loads, so the FLP keeps referencing the workspace. Anything else is refused. The workspace is a path guard, not an OS sandbox against other processes running as your user.
...
Launch defaults to 120 seconds (maximum 300), ordinary bridge calls to 30 seconds, save to 60, and render to 600 (maximum 3600). Embedded execution cancellation signals the plugin and waits for its completion acknowledgement while preserving the pipe and session gate. If the bridge breaks before acknowledgement, FL is left running until a later serialized status reply confirms the interpreter is idle. Already-issued changes cannot be rolled back. Render/launch cancellation stops only processes created by this companion. A render error includes its preserved snapshot path; after explicit client cancellation, use your last known saved recovery path. A partially written output is moved beside itself as `<name>.failed-attempt<n>.wav` (never deleted), so the original filename is free again.
